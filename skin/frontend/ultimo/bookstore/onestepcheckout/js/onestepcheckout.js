...
function oscUpdateCart(element, url, separate_url){
    var qtyElem = element.up('.living_container').select('input').first();
    if(qtyElem){
        ...
        onSuccess: function(transport){
            if(transport.status == 200){
                ...
                if(response.redirect){
                    ...
                    summary.show();
                    if(typeof response.items_count != "undefined" && response.items_count == 0){
                        var submitElems = jQuery('.onestepcheckout-place-order-wrapper button');
                        submitElems.removeClass('validation-passed-btn');
                        jQuery(".checkout_sec").addClass("collapsed").off("click");
                        jQuery(".checkout_sec").next().removeClass("in");
                        submitElems.off("click").on("click",function(){
                            ajaxcart.showError("You need to have products in your cart to checkout, and your cart is empty.");
                        });
                        setLocation(response.base_url);
                        return;
                    }
...
function get_save_billing_function(url, set_methods_url, update_payments, triggered)
{
    ...
    return function()    {
        ...
        parameters["payment[use_internal_credit]"] = $("p_method_customercredit") && $("p_method_customercredit").checked ? 1 : 0;
            ...
            onComplete: function(transport){
                if(transport.status == 200)    {
                    ...
                    if(update_payments){
                        ...
                        $$('div.payment-methods input[name="payment\[use_internal_credit\]"]').invoke('observe', 'change', get_separate_save_methods_function(set_methods_url));
...
function cartBindsAfterAjax(){
    $j('[data-toggle="tooltip"]').tooltip();
}


function formatTelephone(input){
    var curval = $j(input).val();
    switch(input.value.length){
        case 10:
            $j(input).val("("+curval.substr(0,3)+") "+curval.substr(3,3)+'-'+curval.substr(6,4));
            break;
        case 11:
            $j(input).val("+"+curval.substr(0,1)+" ("+curval.substr(1,3)+") "+curval.substr(4,3)+'-'+curval.substr(7,4));
            break;
        case 12:
            $j(input).val("+"+curval.substr(0,2)+" ("+curval.substr(2,3)+") "+curval.substr(5,3)+'-'+curval.substr(8,4));
            break;
    }
}

$j(document).ready(function(){
    $j("#onestepcheckout-form").on('keydown',".validate-telephone",function(e) {
        if ((e.shiftKey || (e.keyCode < 48 || e.keyCode > 57)) && (e.keyCode < 96 || e.keyCode > 105) && e.keyCode !=8 && e.keyCode !=9 ) {
            e.preventDefault();
        }
    });
    $j("#onestepcheckout-form").on('blur',".validate-telephone",function(e) {
        formatTelephone(this);
    });
    $j("#onestepcheckout-form").on('focus',".validate-telephone",function(e) {
