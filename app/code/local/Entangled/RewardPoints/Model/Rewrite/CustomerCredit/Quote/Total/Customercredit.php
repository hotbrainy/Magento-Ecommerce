...
class Entangled_RewardPoints_Model_Rewrite_CustomerCredit_Quote_Total_Customercredit extends MageWorx_CustomerCredit_Model_Quote_Total_Customercredit {
...
//         echo $address->getSubtotal().'---';
        ...
        if ($baseGrandTotal) $baseSubtotal = $baseGrandTotal - $baseShipping; else $baseSubtotal = floatval($address->getBaseSubtotalWithDiscount());
        if ($grandTotal) $subtotal = $grandTotal - $shipping; else $subtotal = floatval($address->getSubtotalWithDiscount());
        ...
        if($address->getDiscountAmount()){
            $baseCreditLeft += $address->getBaseDiscountAmount();
            $creditLeft += $address->getDiscountAmount();
        }
