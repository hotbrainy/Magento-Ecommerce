...
function amshopby_ajax_init(setCollapse = true){
...
function amshopby_ajax_request(url,onSuccess){
    ...
    let mode = jQuery("#view-mode").attr("value");
    urlApi = url.replace("books.html","restcatalog/category/view/id/5");
    var request = new Ajax.Request(urlApi,{
            method: 'get',
            parameters:{'is_ajax':1,'mode': mode},
    ...
    amshopby_update_filter();
}

//update the filter menu based on the URL
function amshopby_update_filter(){

    jQuery(".filter-item-link").each(function(){
        var currentCode = jQuery(this).attr("data-title").replace("amp;","");
        ...
        let adjustParams = {};
        if (params != undefined){
            let attributes = params.split("&");
            for (var i = attributes.length - 1; i > -1; i--) {
                if ( attributes[i].split("=").length != 2){
                    continue;
                }
                let code = attributes[i].split("=")[0];
                adjustParams[code] = [];
                let ids = attributes[i].split("=")[1].split("%2C");
                for (var u = ids.length - 1; u >= 0; u--) {
                    adjustParams[code].push(ids[u]);
                    if (ids[u] == currentId){
                        searching = true;
                        adjustParams[code].pop();
                    }
                }
            }
        }
        ...
        if (searching){
            jQuery("[data-id=" + currentId + "]").addClass("amshopby-attr-selected");
            jQuery("[data-id=" + currentId + "]").removeClass("amshopby-attr");
        }else{
            jQuery("[data-id=" + currentId + "]").removeClass("amshopby-attr-selected");
            jQuery("[data-id=" + currentId + "]").addClass("amshopby-attr");
        }

        //Set the url...
        if (!searching){
            if (adjustParams[currentCode] != undefined){
                adjustParams[currentCode].push(currentId);
            }else{
                adjustParams[currentCode] = [currentId];
            }
        }
        let searchUrl = ""
        for (var i = Object.keys(adjustParams).length - 1; i >= 0; i--) {
            let key = Object.keys(adjustParams)[i];
            if (adjustParams[key].length > 0){
                searchUrl += key + "=" + adjustParams[key].join("%2C") + "&";
            }
        }
        searchUrl = searchUrl.replace(/&\s*$/, "");
        jQuery("[data-id=" + currentId + "]").attr("href", window.location.href.split("?")[0]+"?"+searchUrl );
    });
    ...
    if (params == undefined || params == ""){
        jQuery("#clear-all-bttn").css("display","none");
    }else{
        jQuery("#clear-all-bttn").css("display","block");
    }

    //Active Filter Section
    if (params == undefined || params == ""){
        jQuery("#clear-all-bttn").css("display","none");
        jQuery("#active-filters").css("display","none");
    }else{
        jQuery("#clear-all-bttn").css("display","block");
        jQuery("#active-filters").css("display","block");
        var activeFilterHtml = '<ol>';
        var cnt = 0;
        let attributes = params.split("&");
        for (var i = attributes.length - 1; i > -1; i--) {
            if ( attributes[i].split("=").length != 2){ continue; }
            let code = attributes[i].split("=")[0];
            if (code == "" || (code != "book_imprint" && code != "book_genre" && code != "book_trope")){ continue; }

            let ids = attributes[i].split("=")[1].split("%2C");
            if (ids == ""){ continue; }

            let title = jQuery("[data-title=" + code + "]").first().attr('parent');
            activeFilterHtml += '<li><span class="label">' + title + ':</span>';

            var labels = [];
            var urls = {};
            for (var u = ids.length - 1; u >= 0; u--) {
                if (ids[u] == ""){ continue; }
                let lbl = jQuery("[data-id=" + ids[u] + "]").attr('data-text');
                labels.push(lbl);
                urls[lbl] = window.location.href.replace(ids[u],"").replace("%2C%2C","%2C").replace("=%2C&","=&").replace("=%2C","=").replace("%2C&","&"); //Dirty AF, but works so...
            }
            labels = labels.sort().reverse(); //To Prevent Constant Pos Changes

            for (var u = labels.length - 1; u >= 0; u--) {
                let url = urls[labels[u]];
                activeFilterHtml += '<span class="multiselect-child">';
                    activeFilterHtml += '<a href="' + url + '" class="btn-remove" title="Remove This Item">Remove This Item</a>'
                    activeFilterHtml += labels[u];
        ...
        activeFilterHtml += '</ol>';
        jQuery("#active-filters").html(activeFilterHtml);
    ...
    amshopby_ajax_init(false); //Reset On Clicks
}
...
function amshopby_api_update(data){
    jQuery(".carousel_items").html("")
    let imageBase = "/media/catalog/product";
    let baseUrl = window.location.href.split("?")[0];

    let bookDate = data["books"];
    let filterData = data["filter"];
    let total = data["total_results"];

    //Set Pagination
    let page = parseInt(data["page"]);
    let returnedTotal = data["returned_results"];
    console.log(returnedTotal != total);
    jQuery("[id=result-count]").each(function(){
        if (returnedTotal == total || parseInt(returnedTotal) < 40){
            jQuery(this).html(returnedTotal + " showing(s)");
        }else{
            let startNum = page * parseInt(returnedTotal)-40
           jQuery(this).html((startNum+1) + " to " + (parseInt(returnedTotal)+startNum) + " of " + total);
        }
    });

    nParams = URLToArray(window.location.href);
    nParams["p"] = page + 1;
    lParams = URLToArray(window.location.href);
    lParams["p"] = page - 1;
    let nextUrl = baseUrl + "?" + ArrayToURL(nParams);
    let lastUrl = baseUrl + "?" + ArrayToURL(lParams);
    if (page == 1){
        jQuery("#last-page-bttn").css({color: "#d7d7d7",border: "1px solid #d7d7d7;"})
        jQuery("#last-page-bttn").attr("href","");
        jQuery("#next-page-bttn").attr("href",nextUrl);
    }else{
        jQuery("#last-page-bttn").attr("style","");
        jQuery("#last-page-bttn").attr("href",lastUrl);
        jQuery("#next-page-bttn").attr("href",nextUrl);
    }
    ...
    let mode = jQuery("#view-mode").attr("value");
    console.log(mode);
    bookDate.forEach(function(book) {
        if (mode == "grid"){
            var html = jQuery("#grid-template").html()
            html = html.replace("{TITLE}",book.name);
            html = html.replace("{IMG-URL}",book.small_image);
            html = html.replace("{SHORT-DESC}",book.short_description);
            html = html.replace("{RATING-CNT}",book.rating_count);
            html = html.replace("{RATING}",book.rating_summary + "%");
            html = html.replace(/{PATH}/g,"/" + book.request_path);
            html = html.replace(/SKU/g,book.sku);
            html = html.replace(/{ID}/g,book.entity_id);

            var ribbons = "";
            if (book.on_sale){
                ribbons += '<span class="sticker-wrapper top-right"><span class="sticker sale"></span></span>'
            }
            if (book.new){
                ribbons += '<span class="sticker-wrapper top-left"><span class="sticker new"></span></span>'
            }
            html = html.replace("{RIBBONS}", ribbons);

            jQuery(".carousel_items").append(html)
        }else{
            var html = jQuery("#list-template").html()
            html = html.replace("{TITLE}",book.name);
            html = html.replace("{IMG-URL}",book.small_image);
            html = html.replace("{MED-DESC}",book.medium_description);
            html = html.replace("{RATING-CNT}",book.rating_count);
            html = html.replace("{RATING}",book.rating_summary + "%");
            html = html.replace(/{PATH}/g,"/" + book.request_path);
            html = html.replace(/SKU/g,book.sku);
            ...
            if (book.book_series != null){
                html = html.replace("{SERIES}",book.book_series);
            }else{
                html = html.replace("{SERIES}","&nbsp;&nbsp;&nbsp;");
            }

            if (book.on_sale == false){
                html = html.replace("{PRICE}",parseFloat(book.price).toFixed(2));
            }else{
                html = html.replace("{PRICE}",parseFloat(book.special_price).toFixed(2));
            }
            ...
            if (book.author != undefined){
                if (book.author.length == 0){
                    ...
                    if (book.author.length > 1){
                        html = html.replace("{AUTHOR}",book.author.join(", "));
                    }else{
                        html = html.replace("{AUTHOR}",book.author[0]);
                    }
                }
            }else{
                html = html.replace("{AUTHOR}","");
    ...
    //Adjust Filter Label Counts
    for (var key in filterData){
        var lblCnts = filterData[key];
        if (lblCnts.count == 0){
            jQuery("[data-id=" + lblCnts.id + "]").addClass("amshopby-attr-inactive");
            jQuery("[data-id=" + lblCnts.id + "]").html(lblCnts.label + " (0)");
        }else{
            jQuery("[data-id=" + lblCnts.id + "]").removeClass("amshopby-attr-inactive");
            jQuery("[data-id=" + lblCnts.id + "]").html(lblCnts.label + " (" + lblCnts.count + ")");
        }
    }

    if(typeof data.banner != "undefined"){
        jQuery(".magestore-bannerslider").replaceWith(data.banner.replace("{}",jQuery(".flex-viewport").height()) );
        jQuery(".responsive-image").each(function(){
            if(jQuery("html").hasClass("mobile")){
                var src = jQuery(this).attr("small-src") != "" ? $j(this).attr("small-src") : $j(this).attr("large-src");
            }else{
