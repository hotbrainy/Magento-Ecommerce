...
class Mage_Catalog_Block_Breadcrumbs extends Mage_Core_Block_Template
{
    ...
    protected function _prepareLayout()
    {
        if ($breadcrumbsBlock = $this->getLayout()->getBlock('breadcrumbs')) {
            ...
            if($current_product){
                $imprintVal = $current_product->getAttributeText('book_imprint');
                $imprintVal = is_array($imprintVal) ? $imprintVal[0] : $imprintVal;
                $imprintBreadcrumb = array(
                    'label'=> $imprintVal,
                    'title'=> $imprintVal,
                    'link'=> Mage::getBaseUrl()."books.html?book_imprint=".explode(",",$current_product->getBookImprint())[0]
                );
            }
            ...
            foreach ($path as $name => $breadcrumb) {
                if($name == 'product') $breadcrumbsBlock->addCrumb('imprint', $imprintBreadcrumb);
